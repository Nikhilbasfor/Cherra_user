...
export default function HomePageClient({
  ...
  // Dynamic backend stats, FAQs, and attractions (pre-loaded from server, zero hydration flash)
  ...
  // Automatically derive inventory from currently active hotels
  ...
  useEffect(() => {
    ...
    // 1. Real-time Firestore onSnapshot listeners
    ...
    // 2. BroadcastChannel for sub-millisecond cross-tab updates from Admin panel
    ...
    // 3. Fast fallback refresh polling (every 3s)
    const loadAll = () => {
      getAllHotels()
        .then((data) => {
      ...
      <section className="relative z-10 border-y border-slate-200/80 bg-white/95 backdrop-blur-md py-6 sm:py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 text-center">
            {/* 1. Inventory: Auto-calculated from currently present hotels */}
            <div className="p-3 sm:p-4 rounded-2xl bg-slate-50/80 border border-slate-200/60 shadow-xs">
              ...
              <p className="text-2xl sm:text-3xl font-black text-slate-900">{currentInventory}</p>
              <p className="text-xs text-slate-500 font-medium mt-0.5">Verified Stays in Sohra</p>
            </div>

            {/* 2. Score: Live rating from database */}
            <div className="p-3 sm:p-4 rounded-2xl bg-slate-50/80 border border-slate-200/60 shadow-xs">
            ...
            {/* 3. Pricing: Live pledge from database */}
            <div className="p-3 sm:p-4 rounded-2xl bg-slate-50/80 border border-slate-200/60 shadow-xs">
              <div className="flex items-center justify-center gap-1.5 text-emerald-600 mb-1">
            ...
            {/* 4. Speed: Live response time from database */}
            <div className="p-3 sm:p-4 rounded-2xl bg-slate-50/80 border border-slate-200/60 shadow-xs">
